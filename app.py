...
live_bus_service = LiveBusService()


# ---------------- STOPS ----------------
if menu == "Durakları Listele":
    st.header("📍 Tüm Duraklar")
    for stop in stop_service.get_all_stops():
        st.markdown(
            f"""
        <div style="padding:10px;margin:6px 0;border-radius:10px;
                    background:#111827;color:white;border-left:4px solid #3b82f6;">
            📍 <b>{stop.name}</b><br>🆔 {stop.id}
        </div>
        """,
            unsafe_allow_html=True,
        )
...
# ---------------- BUS LIST ----------------
    ...
    data = live_bus_service.get_live_buses()
    for bus in data:
        st.markdown(
            f"""
        <div style="padding:12px;margin:8px 0;border-radius:12px;
                    background:linear-gradient(135deg,#1f2937,#111827);
                    color:white;border-left:5px solid #3b82f6;">
            🚌 <b>{bus['bus_id']}</b><br>
            🚏 {bus['route']}<br>
            📍 {bus['next_stop']}<br>
            ⏱ {bus['eta']} dk
...
# ---------------- ROUTE ----------------
    ...
    with col1:
        if st.button("⭐ Başlangıç Favorile"):
            save_favorite(stop_names[origin])
            st.success(f"{origin} favorilere eklendi")
    with col2:
        if st.button("⭐ Varış Favorile"):
            save_favorite(stop_names[destination])
            st.success(f"{destination} favorilere eklendi")
        ...
        try:
            ...
            for i, step in enumerate(trip.steps, 1):
                st.markdown(
                    f"""
                <div style="padding:10px;margin:6px 0;border-radius:10px;
                            background:#111827;color:white;border-left:4px solid #4ade80;">
                    <b>{i}. Adım</b><br>
                    🚍 {step.mode}<br>
...
# ---------------- LIVE BUSES ----------------
    ...
    for bus in data:
        ...
        <div style="padding:12px;margin:8px 0;border-radius:12px;
                    background:linear-gradient(135deg,#1f2937,#111827);
                    color:white;border-left:5px solid #4ade80;">
            🚍 {bus['route']}<br>
            🚌 {bus['bus_id']}<br>
            📍 {bus['next_stop']}<br>
            ⏱ {bus['eta']} dk
